	...
	return alexa.ResponseEnvelopeV1(
		...
		alexa.Response{
			OutputSpeech: &alexa.OutputSpeech{
				...
				Text: "このスキルでは、早口言葉の練習をすることができます。準備ができたら、OK、や、次、スタートのように話しかけてください。早口言葉のお題を読み上げますので、読み上げたとおりに繰り返してください。正確に聞き取ることができたら、正解になります。それでは、早口言葉の練習をはじめます。心と口の準備はよろしいですか？",
			},
...
func Launch(alexa.RequestEnvelope) (alexa.ResponseEnvelope, error) {
	...
	return alexa.ResponseEnvelopeV1(
		...
		alexa.Response{
			OutputSpeech: &alexa.OutputSpeech{
				Type: alexa.TypePlainText,
				Text: "早口言葉の練習をはじめます。心と口の準備はよろしいですか？",
			},
			Reprompt: &alexa.Reprompt{
				OutputSpeech: &alexa.OutputSpeech{
					Type: alexa.TypePlainText,
